...
-- Ensure Shirts + Pants subfolders exist
local ShirtsFolder, PantsFolder
if ClothingFolder then
...
local function saveCharacterStuff(char, AccessoryFolder, ShirtsFolder, PantsFolder)
	task.wait(1) -- Short wait for character to fully load
	if not AccessoryFolder or not ShirtsFolder or not PantsFolder then return end
...
for _, player in ipairs(Players:GetPlayers()) do
	player.CharacterAdded:Connect(function(char)
		saveCharacterStuff(char, AccessoryFolder, ShirtsFolder, PantsFolder)
	end)
	if player.Character then
		saveCharacterStuff(player.Character, AccessoryFolder, ShirtsFolder, PantsFolder)
	end
end
